...
class InvertedIndexReader(InvertedIndex):
    ...
    def __next__(self):
        ...
        pos, number_of_postings, len_in_bytes_of_postings, len_in_bytes_of_tf, _, _ = self.postings_dict[
            curr_term]
    ...
    def get_postings_list(self, term):
        ...
        """
        postings_list_location = self.postings_dict[term][0]
        self.index_file.seek(postings_list_location)

        len_in_bytes_of_postings = self.postings_dict[term][2]
        len_in_bytes_of_tf = self.postings_dict[term][3]
    ...
    def get_tfidf_ub(self, term):
        return self.postings_dict[term][4]
    
    def get_bm25_ub(self, term):
        return self.postings_dict[term][5]
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        prev_doc_length = self.avg_doc_length * len(self.doc_length)
        ...
        self.avg_doc_length = (prev_doc_length + sum(tf_list)) / len(self.doc_length)
        

        tfidf_upper_bound = count_tfidf_score(max_tf, len(postings_list), len(self.doc_length))
        bm25_upper_bound = count_bm25_score(
            max_tf, len(postings_list), len(self.doc_length), self.doc_length[max_doc_id], self.avg_doc_length, 1.2, 0.75)
        ...
        self.postings_dict[term] = (
            ...
            len(encoded_tf_list),
            math.ceil(tfidf_upper_bound),
            math.ceil(bm25_upper_bound)
        )
